...
static struct json_object* fm_playlist_send_long_report(fm_playlist_t *pl, int sid, char act)
{
    ...
    sprintf(url, "/usr/bin/curl '%s?app_name=%s&version=%s&user_id=%d&expire=%d&token=%s&channel=%d&sid=%d&type=%c&h=%s'",
    ...
    if(rd) {
       size = fread( ret, 1, 4095, rd);
       pclose(rd);
       if (size>0) {	
      		ret[size]=0; 
		return json_tokener_parse(ret);
	}
    }
	return 0;
...
static void fm_playlist_send_short_report(fm_playlist_t *pl, int sid, char act)
{
    ...
    curl_easy_setopt(pl->curl, CURLOPT_URL, url); curl_easy_setopt(pl->curl, CURLOPT_WRITEFUNCTION, drop_buffer); curl_easy_setopt(pl->curl, CURLOPT_WRITEDATA, NULL); curl_easy_perform(pl->curl);
}
